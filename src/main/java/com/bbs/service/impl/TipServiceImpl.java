...
import com.bbs.mapper.TabMapper;
import com.bbs.mapper.TipMapper;
...
public class TipServiceImpl implements TipService {
    ...
    @Override
    public List<Tip> getMainPageTipsByType(Integer type) {
        Logger logger = Logger.getLogger(TipServiceImpl.class);
        logger.info("处理主页显示的贴子");
        List<Tip> finalTipList = new ArrayList<>(); // 保存最终返回的贴子
        if (type == 1) {    // 置顶
            finalTipList = this.getTipsSolvedElseModel(tipMapper.selAllTopTipForTopTimeDesc());
        } else if (type == 2){ // 活跃
            finalTipList = this.getTipsSolvedElseModel(tipMapper.selAllTipByTypeActive());
        } else if (type == 3){ // 最近
            finalTipList = this.getTipsSolvedElseModel(tipMapper.selAllTipByTypeLately());
        } else if (type ==4){ // 结帖
            finalTipList = this.getTipsSolvedElseModel(tipMapper.selAllTipByTypeStop());
        }
        // 判空
        if (finalTipList != null) {
            return finalTipList;
